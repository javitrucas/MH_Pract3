...
GeneticUtils::crossoverUniformBin(const std::vector<int>& b1,
    ...
    // Genes iguales
    for (int i = 0; i < n; ++i) {
        if (b1[i] == b2[i]) c1[i] = c2[i] = b1[i];
    }

    // Genes distintos
    std::vector<int> idxs;
    for (int i = 0; i < n; ++i) {
        if (b1[i] != b2[i]) idxs.push_back(i);
    }

    static thread_local std::mt19937 rng(std::random_device{}());
    std::shuffle(idxs.begin(), idxs.end(), rng);
    int half = idxs.size() / 2;
...
GeneticUtils::crossoverPositionBin(const std::vector<int>& b1,
    ...
    static thread_local std::mt19937 rng(std::random_device{}());
    std::shuffle(ones1.begin(), ones1.end(), rng);
    std::shuffle(ones2.begin(), ones2.end(), rng);
...
void GeneticUtils::mutateBin(std::vector<int>& bin) const {
    ...
    static thread_local std::mt19937 rng(std::random_device{}());
    std::uniform_int_distribution<int> d1(0, (int)ones.size() - 1);
    std::uniform_int_distribution<int> d0(0, (int)zeros.size() - 1);
